...
EXPECTED_PRODUCER_SHA256 = (
    "e02abc0a7be2ef60c5ddbded0c82bb42ca6af871b30e768c8603ba3798a71b79"
)
EXPECTED_PRODUCER_SIZE_BYTES = 67763
EXPECTED_PRODUCER_LINE_COUNT = 1961
...
def test_executed_producer_source_is_exact_canonical_committed_script() -> None:
    ...
    committed = PRODUCER_MODULE._git_blob_bytes(
        ROOT,
        revision=revision,
        path=PRODUCER_MODULE.PRODUCER_SOURCE_PATH,
    )
    assert resolved == PRODUCER.resolve(strict=True)
...
def test_self_validation_preserves_exact_lf_utf8_bytes(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    class StubValidator:
        ...
        @staticmethod
        def build_diagnostic(**kwargs: Any) -> tuple[
            dict[str, Any], int, Path | None, None
        ]:
            packet_path = kwargs["packet_path"]
            observed["packet"] = packet_path.read_bytes()
            return {"ok": True}, 0, None, None

    def forbidden_write_text(*_args: Any, **_kwargs: Any) -> int:
        raise AssertionError("self-validation must write exact bytes")
    ...
    assert observed["packet"] == rendered.encode("utf-8")
    assert b"\r" not in observed["packet"]
