...
# Independent, dormant R2 requirement-binding check for the local candidate.
# Never import the builder or trust a binding's self-declared expected hash.
# No existing plan/CLI/verification-record path is switched to R2 by this helper.
_R2_LOCAL_REVIEWED_REQUIREMENTS_SHA256 = "19e451b51a7ff1bdb7d8242a1f486f2e8fd410e791516b7dd2ebf9a0dd93ecc1"
_R2_LOCAL_REQUIRED_DEFINITIONS = (
    "authority_boundary",
    "post_run_state_evidence_v1_definition",
    "post_run_state_evidence_v1_limitations",
    "post_run_state_evidence_v1_profile_id",
    "post_run_state_evidence_v1_provider_artifact",
    "post_run_state_evidence_v1_role_obligations",
    "post_run_state_evidence_v1_subject_archives",
)


def _verify_local_r2_requirement_binding(
    binding: dict[str, Any], schema_bytes: bytes, *, expected_schema_sha256: str,
) -> None:
    """Check requirements identity only, not execution, completeness or activation."""
    ...
    _require(actual_digest == expected_schema_sha256, "r2_schema_source_mismatch")
    document = _strict_json_object(schema_bytes, label=SCHEMA_PATH)
    definitions = document.get("$defs")
    _require(type(definitions) is dict, "r2_definition_missing")
    # Enumerate the reviewed closure independently of the producer's graph walk.
    selected: dict[str, Any] = {}
    allowed_refs = {"#/$defs/" + name for name in _R2_LOCAL_REQUIRED_DEFINITIONS}
    for name in _R2_LOCAL_REQUIRED_DEFINITIONS:
        value = definitions.get(name)
        _require(type(value) is dict, "r2_definition_missing")
        selected[name] = value
        ...
        while nodes:
            ...
            if isinstance(node, dict):
                if "$ref" in node:
                    _require(type(node["$ref"]) is str and node["$ref"] in allowed_refs,
                             "r2_definition_reference_invalid")
                nodes.extend(node.values())
            elif isinstance(node, list):
                nodes.extend(node)
    canonical_requirements = _canonical_json_bytes({
        "$ref": "#/$defs/post_run_state_evidence_v1_definition",
        "$defs": selected,
    })
    _require(hashlib.sha256(canonical_requirements).hexdigest()
             == _R2_LOCAL_REVIEWED_REQUIREMENTS_SHA256, "r2_requirements_changed")
    _require(type(binding) is dict, "r2_binding_not_object")
    expected = {
        "binding_version": "pulsemech_step5c_r2_local_requirements_binding_v0",
        "candidate_scope": "local_only_not_activated",
        "evidence_profile": "pulsemech_step5c_post_run_state_evidence_v1",
        "topology_profile": "pulse_ci_hosted_release_grade_v0",
        "requirements_sha256": _R2_LOCAL_REVIEWED_REQUIREMENTS_SHA256,
        "schema_source": {
            "path": SCHEMA_PATH,
    ...
    # Canonical byte comparison also rejects 0/False and 1/True substitutions.
    _require(_canonical_json_bytes(binding) == _canonical_json_bytes(expected),
             "r2_binding_mismatch")
...
def _verify_source_recorded_equations(plan: dict[str, Any], workflow: dict[str, Any],
    ...
    for role in ("pre-materialization-status", "recorded-release-candidate-envelopes"):
        ...
        if role == "pre-materialization-status":
            ...
            expected_consumers.update(oid for oid, eq in _source_residual_input_expectations(workflow, source_by_path)["steps"].items()
            ...
            publication = _source_recorded_publication_selectors(workflow, source_by_path)
            _require(publication["tree_selectors"] == [facts["locators"][role] + "**"],
                     "recorded_publication_tree_selector_mismatch")
            expected_consumers.add(publication["occurrence_id"])
            report = _source_report_publication_selectors(workflow, source_by_path)
            _require(_report_publication_covers_locator(report["artifact_tree_root"], facts["locators"][role]),
                     "report_publication_candidate_selector_mismatch")
            expected_consumers.add(report["occurrence_id"])
        role_id = sid(role)
...
def _verify_source_provenance_equations(plan: dict[str, Any], workflow: dict[str, Any], sources: dict[str, GitObject]) -> None:
    ...
    binding = states[sid]
    # Extend this formerly closed family only by the independently checked
    # R31 tree selector. Arbitrary additional consumers remain rejected.
    report = _source_report_publication_selectors(workflow, sources)
    _require(_report_publication_covers_locator(report["artifact_tree_root"], facts["locator"]),
             "report_publication_provenance_selector_mismatch")
    expected_consumers = sorted(set(facts["consumers"]) | {report["occurrence_id"]})
    ...
    _require(binding["required_consumer_occurrence_ids"] == expected_consumers, "provenance_mapping_consumers_mismatch")
    ...
    _require(sorted(oid for oid, step in steps.items() if sid in step["input_state_ids"]) == expected_consumers,
...
def _source_baseline_floor_expectations(workflow: dict[str, Any], sources: dict[str, GitObject]) -> dict[str, Any]:
    """Check the floor artifact references, independently of builder loaders."""
    for path, expected in _FLOOR_SOURCE_PINS.items():
        obj = sources.get(path)
        _require(obj is not None and obj.path == path, "floor_mapping_source_missing", path)
        _require(_sha1_git_blob(obj.data) == expected, "floor_mapping_source_drift", path)
    ...
    _require(any(isinstance(n, ast.Assign) and ast.dump(n.targets[0]) == ast.dump(ast.Name(id="p", ctx=ast.Store()))
                 and isinstance(n.value, ast.Constant) and n.value.value == status for n in inline.body),
             "floor_mapping_status_guard_read")
    _require('json.load(open(p,"r",encoding="utf-8"))' in snippet, "floor_mapping_status_guard_read")
    nodes = {n.name: n for n in ast.parse(sources[_FLOOR_BUILD_PATH].data).body if isinstance(n, ast.FunctionDef)}
    refs = [(n.args[1].value, n.args[2].id) for n in ast.walk(nodes["build_floor"])
            if isinstance(n, ast.Call) and isinstance(n.func, ast.Name) and n.func.id == "_artifact"
            and len(n.args) == 3 and isinstance(n.args[1], ast.Constant) and isinstance(n.args[2], ast.Name)]
    _require(sorted(refs) == sorted([("status", "status_path"), ("gate_policy", "policy_path"),
                                    ("gate_registry", "registry_path"), ("required_gate_evidence", "required_evidence_path")]),
             "floor_mapping_artifact_references")
    # The reference helper hashes the actual referenced file; it is not only a
    # recorded name. Source bytes are pinned above; no source function is run.
    _require(any(isinstance(n, ast.Call) and isinstance(n.func, ast.Name) and n.func.id == "_sha256"
                 and [a.id for a in n.args if isinstance(a, ast.Name)] == ["repo", "path", "role"]
                 for n in ast.walk(nodes["_artifact"])), "floor_mapping_artifact_hash")
    paths = {"status": status, "gate_policy": _checked_mapping_path(call["--policy"]),
             "gate_registry": _checked_mapping_path(call["--registry"]),
             "required_gate_evidence": _checked_mapping_path(call["--required-gate-evidence"])}
    floor = _checked_mapping_path(call["--out"])
    _require(paths["gate_policy"] == POLICY_PATH and paths["gate_registry"] == REGISTRY_PATH
...
def _verify_source_baseline_floor_equations(plan: dict[str, Any], workflow: dict[str, Any], sources: dict[str, GitObject]) -> None:
    """Reject common-mode table errors before independent-plan equality."""
    facts = _source_baseline_floor_expectations(workflow, sources)
    states = {row["state_id"].removeprefix("state:step5c:"): row for row in plan["state_templates"]}
    _require(len(states) == len(plan["state_templates"]), "floor_mapping_duplicate_role")
    occurrences = [step for job in plan["jobs"] for step in job["steps"]]
    steps = {step["occurrence_id"]: step for step in occurrences}
    _require(len(steps) == len(occurrences), "floor_mapping_duplicate_step")
    for role, path in facts["locators"].items():
        _require(role in states, "floor_mapping_role_missing", role)
        _require(states[role]["path_or_uri"] == path, "floor_mapping_locator_mismatch", role)
    for role, producer in facts["producers"].items():
        row = states[role]
        _require(row["producer_occurrence_id"] == producer, "floor_mapping_producer_mismatch", role)
        _require(row["required"] is True and row["content_requirement"] == "exact_digest"
                 and row["authority_bearing"] is True and row["mutation_class"] == "none"
                 and row["state_type"] == ("status" if role == "status-baseline" else "release_evidence")
                 and row["role"] == ("pre_augmentation_status" if role == "status-baseline" else "self_contained_pulse_evidence_floor"),
                 "floor_mapping_requirement_mismatch", role)
        sid = row["state_id"]
        _require([oid for oid, step in steps.items() if sid in step["output_state_ids"]] == [producer],
                 "floor_mapping_writer_set_mismatch", role)
    for oid, equation in facts["steps"].items():
        _require(oid in steps, "floor_mapping_step_missing", oid)
        for field, direction in (("input_state_ids", "inputs"), ("output_state_ids", "outputs")):
            expected = sorted("state:step5c:" + role for role in equation[direction])
            _require(steps[oid][field] == expected, "floor_mapping_step_io_mismatch", oid)
        actual_readers = sorted(row["state_id"] for row in states.values() if oid in row["required_consumer_occurrence_ids"])
...
# Closed semantic role labels for the selected source filenames. Derive each
# path from the restore/hash loop and check the corresponding upload independently.
...
def _source_llamaguard_preservation_expectations(workflow: dict[str, Any], sources: dict[str, GitObject]) -> dict[str, Any]:
    """Derive content reads from hash loops, then verify transport and copies."""
    source = sources.get(SUBJECT_WORKFLOW_PATH)
    _require(source is not None and source.path == SUBJECT_WORKFLOW_PATH, "lg_preservation_source_missing")
    _require(_sha1_git_blob(source.data) == EXPECTED_SUBJECT_WORKFLOW_BLOB_SHA1, "lg_preservation_source_drift")
    _require(_parse_yaml_document(source.data, label=SUBJECT_WORKFLOW_PATH) == workflow,
    ...
    for reader_job, reader_no, writer_job, writer_no, variable, count in (
        ...
        publisher = jobs[writer_job]["steps"][writer_no - 1]
        body = reader["run"]
        flat = body.replace("\\\n", " ").splitlines()
        root = _checked_mapping_export(reader, variable)
        download_root = _checked_mapping_export(reader, "DOWNLOAD_DIR", allow_temp=True)
        headers = [shlex.split(line)[3:] for line in flat if line.startswith("for artifact in ")]
        _require(len(headers) == 1, "lg_preservation_hash_loop")
        digest_paths = [_checked_mapping_path(word.replace("${" + variable + "}/", root + "/")) for word in headers[0]]
        basenames = [path.rsplit("/", 1)[-1] for path in digest_paths]
        role_names = list(_LG_PRESERVATION_FILES)[:count]
        _require(len(digest_paths) == len(set(digest_paths)) == len(set(basenames)) == count
                 and basenames == role_names, "lg_preservation_hash_members")
        _require(re.findall(r'^  sha256sum (.*)$', body, re.M) == ['"${artifact}"'], "lg_preservation_hash_call")
        guard = '! -f "${artifact}" || -L "${artifact}"' + (' || ! -s "${artifact}"' if count == 6 else '')
        _require('if [[ ' + guard + ' ]]; then' in body, "lg_preservation_nonempty_guard")
        if count == 3:
            name_headers = [shlex.split(line)[3:] for line in flat if line.startswith("for name in ")]
            _require(name_headers == [basenames]
                     and re.findall(r'^  cp (.*)$', body, re.M) == ['"${src}" "${CANONICAL_DIR}/${name}"'],
                     "lg_preservation_copy_members")
            copied_paths = [root + "/" + name for name in name_headers[0]]
        else:
            calls = re.findall(r'^restore_external_artifact\s+"([A-Za-z0-9_.-]+)"\s+"([^"\n]+)"\s*$',
                               body.replace("\\\n", " "), re.M)
            copied_paths = [_checked_mapping_path(target.replace("${CANONICAL_EXTERNAL}/", root + "/")) for _, target in calls]
            _require([name for name, _ in calls] == basenames, "lg_preservation_copy_members")
            function = re.search(r'^restore_external_artifact \(\) \{\n(.*?)^\}', body, re.M | re.S)
            _require(function is not None and 'local name="$1"' in function.group(1)
                     and 'local dst="$2"' in function.group(1)
                     and re.findall(r'^  cp (.*)$', function.group(1), re.M) == ['"${src}" "${dst}"'],
                     "lg_preservation_copy_destination")
        _require(copied_paths == digest_paths
                 and 'find "${DOWNLOAD_DIR}" -type f -name "${name}" | head -n 1 || true' in body,
                 "lg_preservation_copy_source")
        argv = [shlex.split(line) for line in flat if line.startswith("gh run download ")]
        _require(len(argv) == 1 and len(argv[0]) == 10
                 and argv[0][:4] == ["gh", "run", "download", "${GITHUB_RUN_ID}"]
                 and argv[0][4:7] == ["--repo", "${GITHUB_REPOSITORY}", "--name"]
                 and argv[0][8:] == ["--dir", "${DOWNLOAD_DIR}"], "lg_preservation_download_context")
        name = argv[0][7].replace("${GITHUB_RUN_ID}", "{workflow_run_id}").replace("${GITHUB_RUN_ATTEMPT}", "1")
        options = publisher["with"]
        upload_name = options["name"].replace("${{ github.run_id }}", "{workflow_run_id}").replace("${{ github.run_attempt }}", "1")
        uploaded = [_checked_mapping_path(line) for line in options["path"].splitlines()]
        _require(name == upload_name and re.fullmatch(r'[a-z0-9-]+-\{workflow_run_id\}-1', name) is not None,
                 "lg_preservation_artifact_name")
        _require(publisher.get("uses") == "actions/upload-artifact@043fb46d1a93c77aae656e7c1c64a875d1fc6a0a"
                 and options.get("if-no-files-found") == "error", "lg_preservation_upload_profile")
        _require(uploaded == digest_paths, "lg_preservation_upload_members")
        for leaf, path in zip(basenames, digest_paths):
            role = _LG_PRESERVATION_FILES[leaf]
            _require(role not in local or local[role] == path, "lg_preservation_version_alias")
            local[role] = path
        upload_id, reader_id = _step_id(writer_job, writer_no), _step_id(reader_job, reader_no)
        inputs = sorted(_LG_PRESERVATION_FILES[leaf] for leaf in basenames)
        duties[upload_id] = {"inputs": inputs, "outputs": []}
        duties[reader_id] = {"inputs": inputs, "outputs": []}
        handoffs.append({"publisher": upload_id, "restorer": reader_id, "artifact_name_template": name,
                         "upload_paths": uploaded, "restore_paths": copied_paths, "hash_paths": digest_paths,
    ...
    return {"locators": local, "origins": content_origins, "steps": duties, "handoffs": handoffs}
...
def _verify_source_llamaguard_preservation_equations(plan: dict[str, Any], workflow: dict[str, Any], sources: dict[str, GitObject]) -> None:
    """Reject incorrect submitted source relations even if constructors agree."""
    facts = _source_llamaguard_preservation_expectations(workflow, sources)
    states = {row["state_id"].removeprefix("state:step5c:"): row for row in plan["state_templates"]}
    _require(len(states) == len(plan["state_templates"]), "lg_preservation_duplicate_role")
    occurrences = [step for job in plan["jobs"] for step in job["steps"]]
    steps = {step["occurrence_id"]: step for step in occurrences}
    _require(len(steps) == len(occurrences), "lg_preservation_duplicate_step")
    for role, path in facts["locators"].items():
        _require(role in states, "lg_preservation_role_missing", role)
        row = states[role]
        _require(row["path_or_uri"] == path, "lg_preservation_locator_mismatch", role)
        _require(row["required"] is True and row["content_requirement"] == "exact_digest"
                 and row["authority_bearing"] is True and row["mutation_class"] == "none",
                 "lg_preservation_requirement_mismatch", role)
        _require(row["producer_occurrence_id"] == facts["origins"][role], "lg_preservation_origin_mismatch", role)
        _require([oid for oid, step in steps.items() if row["state_id"] in step["output_state_ids"]]
                 == [facts["origins"][role]], "lg_preservation_writer_set_mismatch", role)
    for oid, equation in facts["steps"].items():
        _require(oid in steps, "lg_preservation_step_missing", oid)
        for field, direction in (("input_state_ids", "inputs"), ("output_state_ids", "outputs")):
            _require(steps[oid][field] == sorted("state:step5c:" + role for role in equation[direction]),
                     "lg_preservation_step_io_mismatch", oid + ":" + direction)
        _require(sorted(row["state_id"] for row in states.values() if oid in row["required_consumer_occurrence_ids"])
...
_LG_PRODUCTION_PINS = {
    ...
    LLAMAGUARD_RUNNER_PATH: EXPECTED_LLAMAGUARD_RUNNER_BLOB_SHA1,
    LLAMAGUARD_DATASET_PATH: EXPECTED_DATASET_BLOB_SHA1,
    _LG_INGEST_PATH: "b1416fb3675a3d4bb652eaa993ed14a51a96f9c2",
}
...
# R33's declaration is not the acquired artifact or an observed member list.
def _source_recorded_publication_selectors(
    workflow: dict[str, Any], sources: dict[str, GitObject],
) -> dict[str, Any]:
    source = sources.get(SUBJECT_WORKFLOW_PATH)
    ...
    job = "release_grade_recorded_path"
    uploads = [(n, step) for n, step in enumerate(workflow["jobs"][job]["steps"], 1)
               if step.get("name") == "Upload release-grade recorded path artifacts"]
    _require(len(uploads) == 1 and uploads[0][0] == 33, "recorded_publication_step_profile")
    number, upload = uploads[0]
    _require(sorted(upload) == ["name", "uses", "with"]
             and upload.get("uses") == "actions/upload-artifact@043fb46d1a93c77aae656e7c1c64a875d1fc6a0a",
             "recorded_publication_action_profile")
    fields = upload["with"]
    _require(sorted(fields) == ["if-no-files-found", "name", "path", "retention-days"]
             and fields.get("retention-days") == "30" and fields.get("if-no-files-found") == "error"
             and fields.get("name") == "release-grade-recorded-path-${{ github.run_id }}-${{ github.run_attempt }}",
             "recorded_publication_options_profile")
    parsed = []
    for line in fields["path"].splitlines():
        match = re.fullmatch(r"(?P<path>PULSE_safe_pack_v0/[A-Za-z0-9_./-]+?)(?P<glob>/\*\*)?", line)
        _require(match is not None, "recorded_publication_selector_alias")
        physical = _checked_mapping_path(match.group("path"))
        parsed.append((physical, bool(match.group("glob"))))
    ordered = [path + ("/**" if tree else "") for path, tree in parsed]
    trees = [path + "/**" for path, tree in parsed if tree]
    files = [path for path, tree in parsed if not tree]
    _require(len(set(ordered)) == len(ordered) == 27 and len(trees) == 1 and len(files) == 26,
             "recorded_publication_selector_extent")
    return {"occurrence_id": _step_id(job, number), "action_uses": upload["uses"],
            "artifact_name_template": fields["name"], "ordered_selectors": ordered,
            "exact_file_selectors": files, "tree_selectors": trees,
...
def _source_recorded_publication_expectations(
    ...
    """Resolve each exact file/tree selector through independently read sources."""
    selectors = _source_recorded_publication_selectors(workflow, sources)
    final = _source_final_artifact_postcondition_expectations(workflow, sources)
    floor = _source_baseline_floor_expectations(workflow, sources)
    records = _source_recorded_expectations(workflow, sources)
    floor_role, tree_role = "self-contained-evidence-floor", "recorded-release-candidate-envelopes"
    candidate_root = records["locators"][tree_role]
    _require(candidate_root.endswith("/") and selectors["tree_selectors"] == [candidate_root + "**"],
             "recorded_publication_tree_selector_mismatch")
    extra_files = set(selectors["exact_file_selectors"]) - set(final["checked_paths"])
    _require(extra_files == {floor["locators"][floor_role]}
             and set(final["checked_paths"]) <= set(selectors["exact_file_selectors"]),
             "recorded_publication_file_correspondence")
    families = [final["locators"], {floor_role: floor["locators"][floor_role]}]
    locators, origins, role_selectors = {}, {}, {}
    for path in selectors["exact_file_selectors"]:
        matches = [(role, locator) for family in families for role, locator in family.items() if locator == path]
        _require(len(matches) <= 1, "recorded_publication_selected_extent", path)
        if matches:
            role, locator = matches[0]
            _require(role not in locators, "recorded_publication_selected_extent", role)
            locators[role] = role_selectors[role] = locator
            if role in final["origins"]:
                origins[role] = final["origins"][role]
            else:
                writers = {oid for oid, eq in floor["steps"].items() if role in eq["outputs"]}
                _require(len(writers) == 1, "recorded_publication_source_origin_extent", role)
                origins[role] = writers.pop()
    locators[tree_role] = candidate_root
    role_selectors[tree_role] = selectors["tree_selectors"][0]
    writers = {oid for oid, eq in records["steps"].items() if tree_role in eq["outputs"]}
    _require(len(writers) == 1, "recorded_publication_source_origin_extent", tree_role)
    origins[tree_role] = writers.pop()
    _require(len(locators) == len(origins) == len(set(role_selectors.values())) == 23,
             "recorded_publication_selected_extent")
    unmodeled = sorted(path for path in selectors["exact_file_selectors"] if path not in locators.values())
    ...
    return {**selectors, "locators": locators, "origins": origins, "role_selectors": role_selectors,
            "unmodeled_file_selectors": unmodeled, "publication_only_roles": sorted([floor_role, tree_role]),
...
def _install_recorded_publication_projection(
    ...
    """Reconstruct selected upload dependencies, retaining every state duty."""
    oid = facts["occurrence_id"]
    step = steps[("release_grade_recorded_path", 33)]
    _require(step["occurrence_id"] == oid and not step["output_state_ids"],
             "recorded_publication_not_a_content_writer")
    indexed = {row["state_id"]: row for row in states}
    for role in facts["locators"]:
        sid = "state:step5c:" + role
        _require(sid in indexed and indexed[sid]["path_or_uri"] == facts["locators"][role],
                 "recorded_publication_input_locator", role)
        _require(indexed[sid]["producer_occurrence_id"] == facts["origins"][role],
                 "recorded_publication_input_origin", role)
    step["input_state_ids"] = sorted("state:step5c:" + role for role in facts["locators"])
    for row in states:
        readers = {reader for reader in row["required_consumer_occurrence_ids"] if reader != oid}
        if row["state_id"] in step["input_state_ids"]:
            readers.add(oid)
        row["required_consumer_occurrence_ids"] = sorted(readers)
...
def _verify_source_recorded_publication_equations(
    ...
    """Reject false publication inputs even when both constructors agree."""
    facts = _source_recorded_publication_expectations(workflow, sources)
    states = {row["state_id"].removeprefix("state:step5c:"): row for row in plan["state_templates"]}
    _require(len(states) == len(plan["state_templates"]), "recorded_publication_duplicate_role")
    occurrences = [step for job in plan["jobs"] for step in job["steps"]]
    steps = {step["occurrence_id"]: step for step in occurrences}
    _require(len(steps) == len(occurrences), "recorded_publication_duplicate_step")
    for role, path in facts["locators"].items():
        _require(role in states, "recorded_publication_role_missing", role)
        row = states[role]
        _require(row["path_or_uri"] == path, "recorded_publication_locator_mismatch", role)
        _require(row["producer_occurrence_id"] == facts["origins"][role],
                 "recorded_publication_origin_mismatch", role)
        mutation = {"final-status": "final_status", "release-decision": "release_decision"}.get(role, "none")
        authority = role not in ("release-grade-junit", "release-grade-sarif")
        _require(row["required"] is True and row["content_requirement"] == "exact_digest"
                 and row["authority_bearing"] is authority and row["mutation_class"] == mutation,
                 "recorded_publication_duty_mismatch", role)
        writers = {oid for oid, step in steps.items() if row["state_id"] in step["output_state_ids"]}
        _require(writers == {facts["origins"][role]}, "recorded_publication_writer_mismatch", role)
    oid = facts["occurrence_id"]
    _require(oid in steps, "recorded_publication_step_missing")
    _require(steps[oid]["input_state_ids"] == sorted("state:step5c:" + role for role in facts["locators"])
             and steps[oid]["output_state_ids"] == [], "recorded_publication_step_io_mismatch")
    _require(sorted(row["state_id"] for row in states.values() if oid in row["required_consumer_occurrence_ids"])
...
def _source_report_publication_selectors(
    workflow: dict[str, Any], sources: dict[str, GitObject],
) -> dict[str, Any]:
    """Extract the pinned R31 publication, independently of the plan builder."""
    obj = sources.get(SUBJECT_WORKFLOW_PATH)
    _require(obj is not None and obj.path == SUBJECT_WORKFLOW_PATH,
             "report_publication_source_missing")
    digest = hashlib.sha1(b"blob " + str(len(obj.data)).encode() + b"\0" + obj.data).hexdigest()
    _require(digest == EXPECTED_SUBJECT_WORKFLOW_BLOB_SHA1, "report_publication_source_drift")
    original = _parse_yaml_document(obj.data, label=SUBJECT_WORKFLOW_PATH)
    _require(original == workflow, "report_publication_workflow_drift")
    job = "release_grade_recorded_path"
    matches = [(n, step) for n, step in enumerate(original["jobs"][job]["steps"], 1)
               if step.get("with", {}).get("name") == "pulse-report"]
    _require(len(matches) == 1 and matches[0][0] == 31, "report_publication_step_profile")
    position, step = matches[0]
    _require(set(step.keys()) == {"name", "uses", "with"}
             and step["name"] == "Upload final pulse report", "report_publication_action_profile")
    action, separator, revision = step["uses"].partition("@")
    _require(action == "actions/upload-artifact" and separator == "@"
             and revision == "043fb46d1a93c77aae656e7c1c64a875d1fc6a0a",
             "report_publication_action_profile")
    opts = step["with"]
    _require(set(opts) == {"path", "name", "retention-days", "if-no-files-found"}
             and opts["if-no-files-found"] == "error" and opts["retention-days"] == "30",
             "report_publication_options_profile")
    tokens = shlex.split(opts["path"], posix=True, comments=False)
    _require(tokens == opts["path"].splitlines() and len(tokens) == len(set(tokens)) == 4,
             "report_publication_selector_profile")
    parts = [token.split("/") for token in tokens]
    _require(parts == [["PULSE_safe_pack_v0", "artifacts", "**"], ["badges", "*.svg"],
                       ["reports", "junit.xml"], ["reports", "sarif.json"]],
             "report_publication_selector_profile")
    return {"occurrence_id": _step_id(job, position), "artifact_name": opts["name"],
            "action_uses": step["uses"], "ordered_selectors": tokens,
            "artifact_tree_root": "/".join(parts[0][:-1]) + "/",
            "artifact_tree_selector": tokens[0], "badge_selector": tokens[1],
            "root_report_selectors": tokens[2:], "if_no_files_found": "error", "retention_days": 30}


def _report_publication_covers_locator(root: str, locator: str) -> bool:
    """Check selected physical-path ancestry, without globbing or alias removal."""
    if not isinstance(locator, str) or root.split("/") != ["PULSE_safe_pack_v0", "artifacts", ""]:
        return False
    parts = locator.split("/")
    if parts[-1:] == [""]:
        parts = parts[:-1]
    if len(parts) < 3 or parts[:2] != ["PULSE_safe_pack_v0", "artifacts"]:
        return False
...
def _source_report_publication_expectations(
    ...
    """Resolve prior final source roles against R31's selected tree boundary."""
    selectors = _source_report_publication_selectors(workflow, sources)
    recorded = _source_recorded_publication_expectations(workflow, sources)
    bundle = _source_bundle_expectations(workflow, sources)
    audit = "release-authority-audit-bundle"
    roles = sorted(set(recorded["locators"]) | {audit})
    paths: dict[str, str] = {}
    origins: dict[str, str] = {}
    for role in roles:
        path = bundle["locators"][role] if role == audit else recorded["locators"][role]
        _require(_report_publication_covers_locator(selectors["artifact_tree_root"], path),
                 "report_publication_selected_locator", role)
        paths[role] = path
        origins[role] = bundle["producers"][role] if role == audit else recorded["origins"][role]
    _require(len(paths) == len(set(paths.values())) == 24, "report_publication_selected_extent")
    trees = [role for role in roles if paths[role].endswith("/")]
    _require(trees == ["recorded-release-candidate-envelopes", audit], "report_publication_tree_roles")
    known_unmodeled = sorted(recorded["unmodeled_file_selectors"])
    ...
    _require(all(path not in selectors["root_report_selectors"] for path in paths.values()),
             "report_publication_root_report_alias")
    return {**selectors, "locators": paths, "origins": origins,
            "role_selectors": dict.fromkeys(roles, selectors["artifact_tree_selector"]),
            "tree_roles": trees, "known_unmodeled_artifact_paths": known_unmodeled,
            "unmodeled_literal_selectors": selectors["root_report_selectors"][:],
            ...
            "read_basis": "source_declared_publication_input", "observed_read_receipt": False,
            "archive_membership_observed": False, "semantic_content_admission": False,
            "steps": {selectors["occurrence_id"]: {"inputs": roles, "outputs": []}}}


def _install_report_publication_projection(
    states: list[dict[str, Any]], steps: dict[tuple[str, int], dict[str, Any]], facts: dict[str, Any],
) -> None:
    """Reconstruct selected R31 consumers while keeping their original writers."""
    selected = sorted("state:step5c:" + role for role in facts["locators"])
    row = steps[("release_grade_recorded_path", 31)]
    oid = facts["occurrence_id"]
    _require(row["occurrence_id"] == oid and not row["output_state_ids"],
             "report_publication_not_a_content_writer")
    indexed = {state["state_id"]: state for state in states}
    for role in facts["locators"]:
        sid = "state:step5c:" + role
        _require(sid in indexed and indexed[sid]["path_or_uri"] == facts["locators"][role],
    ...
    row["input_state_ids"] = selected
    for state in states:
        remaining = [r for r in state["required_consumer_occurrence_ids"] if r != oid]
        state["required_consumer_occurrence_ids"] = sorted(set(
...
def _verify_source_report_publication_equations(
    ...
    """Check R31 relations against source before canonical reconstruction equality."""
    facts = _source_report_publication_expectations(workflow, sources)
    states = {row["state_id"].removeprefix("state:step5c:"): row for row in plan["state_templates"]}
    _require(len(states) == len(plan["state_templates"]), "report_publication_duplicate_role")
    occurrences = [step for job in plan["jobs"] for step in job["steps"]]
    steps = {step["occurrence_id"]: step for step in occurrences}
    _require(len(steps) == len(occurrences), "report_publication_duplicate_step")
    for role, path in facts["locators"].items():
        _require(role in states, "report_publication_role_missing", role)
        row = states[role]
        _require(row["path_or_uri"] == path, "report_publication_locator_mismatch", role)
        _require(row["producer_occurrence_id"] == facts["origins"][role],
                 "report_publication_origin_mismatch", role)
        mutation = {"final-status": "final_status", "release-decision": "release_decision"}.get(role, "none")
        authority = role not in ("release-grade-junit", "release-grade-sarif")
        _require(row["required"] is True and row["content_requirement"] == "exact_digest"
                 and row["authority_bearing"] is authority and row["mutation_class"] == mutation,
                 "report_publication_duty_mismatch", role)
        _require({oid for oid, step in steps.items() if row["state_id"] in step["output_state_ids"]}
                 == {facts["origins"][role]}, "report_publication_writer_mismatch", role)
    oid = facts["occurrence_id"]
    _require(oid in steps, "report_publication_step_missing")
    inputs = sorted("state:step5c:" + role for role in facts["locators"])
    _require(steps[oid]["input_state_ids"] == inputs and steps[oid]["output_state_ids"] == [],
             "report_publication_step_io_mismatch")
    _require(sorted(row["state_id"] for row in states.values() if oid in row["required_consumer_occurrence_ids"])
             == inputs, "report_publication_reverse_inputs_mismatch")
...
def _source_residual_input_expectations(
    ...
    """Independent source/path equations, not a copied producer result."""
    wf = sources.get(SUBJECT_WORKFLOW_PATH)
    _require(wf is not None and wf.path == SUBJECT_WORKFLOW_PATH, "residual_input_source_missing")
    _require(_sha1_git_blob(wf.data) == EXPECTED_SUBJECT_WORKFLOW_BLOB_SHA1,
             "residual_input_source_drift", SUBJECT_WORKFLOW_PATH)
    parsed = _parse_yaml_document(wf.data, label=SUBJECT_WORKFLOW_PATH)
    _require(workflow == parsed, "residual_input_workflow_drift")
    for relative in sorted(_RESIDUAL_SOURCE_PINS):
        obj = sources.get(relative)
        _require(obj is not None and obj.path == relative, "residual_input_source_missing", relative)
        _require(_sha1_git_blob(obj.data) == _RESIDUAL_SOURCE_PINS[relative], "residual_input_source_drift", relative)
    pack = "PULSE_safe_pack_v0/tools/"
    ...
    def literal(path: str, key: str) -> str:
        ...
        _require(isinstance(value, str), "residual_input_literal_not_text", key)
        return _checked_mapping_path(value)
    final = literal(candidate, "STATUS")
    pre = final + "#pre-release-required-materialization"
    evidence = literal(candidate, "REQUIRED_EVIDENCE")
    _require(literal(origin, "EVIDENCE_PATH") == evidence, "residual_input_evidence_handoff")
    policy, registry, threshold = (literal(candidate, k) for k in ("POLICY", "REGISTRY", "THRESHOLDS"))
    _require(policy == POLICY_PATH == literal(origin, "POLICY_PATH")
             and registry == REGISTRY_PATH == literal(origin, "REGISTRY_PATH"), "residual_input_policy_handoff")
    ...
    exporter = rows[22]
    status_env = _checked_mapping_export(exporter, "PULSE_STATUS")
    junit = _checked_mapping_export(exporter, "PULSE_JUNIT")
    sarif = _checked_mapping_export(exporter, "PULSE_SARIF")
    _require(status_env == final and junit == "PULSE_safe_pack_v0/artifacts/reports/junit.xml"
             and sarif == "PULSE_safe_pack_v0/artifacts/reports/sarif.json", "residual_input_export_paths")
    pol_calls = _recorded_source_commands(exporter, "tools/policy_to_require_args.py", ("--policy", "--set", "--format"), 2)
    _require(tuple(c["--set"] for c in pol_calls) == ("required", "release_required")
             and all(c["--policy"] == policy and c["--format"] == "newline" for c in pol_calls),
             "residual_input_export_policy_order")
    _require(exporter["env"] == {"PULSE_EVENT_NAME": "${{ github.event_name }}",
             "PULSE_REF": "${{ github.ref }}", "PULSE_SHA": "${{ github.sha }}"}, "residual_input_export_identity")
    ext = literal(candidate, "EXTERNAL_DIR") + "/"
    manifest = _checked_mapping_path(_recorded_source_commands(rows[7], pack + "check_recorded_release_evidence_v0.py",
                    ("--manifest", "--repo-root", "--out-json"))[0]["--manifest"])
    verification = _checked_mapping_path(_recorded_source_commands(rows[7], pack + "check_recorded_release_evidence_v0.py",
                    ("--manifest", "--repo-root", "--out-json"))[0]["--out-json"])
    names = {SUBJECT_WORKFLOW_PATH: "workflow-source", policy: "gate-policy", registry: "gate-registry",
             pre: "pre-materialization-status", final: "final-status", evidence: "required-gate-evidence",
             threshold: "threshold-policy", EXTERNAL_SIGNER_POLICY_PATH: "external-signer-policy",
             literal(candidate, "INDEX"): "recorded-candidate-index",
             literal(candidate, "OUT_DIR") + "/": "recorded-release-candidate-envelopes",
             manifest: "release-evidence-input-manifest", verification: "recorded-release-evidence-verifier",
             junit: "release-grade-junit", sarif: "release-grade-sarif",
             ext + "llamaguard_summary.json": "llamaguard-summary",
             ext + "llamaguard_raw.jsonl": "llamaguard-raw-evidence",
             ext + "llamaguard_summary.envelope.json": "llamaguard-attestation-envelope",
             ext + "llamaguard_summary.bundle.json": "llamaguard-attestation-bundle",
             ext + "llamaguard_attestation_verifier_v1.json": "llamaguard-attestation-verifier"}
    _require(len(names) == 19, "residual_input_path_alias")
    # Whole-file semantic pins bind the implemented replay call graph. The
    # returned in-memory attestation report is not a read of the L7 output.
    common = (pre, evidence, policy, registry, threshold, EXTERNAL_SIGNER_POLICY_PATH,
              ext + "llamaguard_summary.json", ext + "llamaguard_raw.jsonl",
              ext + "llamaguard_summary.envelope.json", ext + "llamaguard_summary.bundle.json")
    ...
    for job in _RESIDUAL_CHECKOUT_JOBS:
        raw = parsed["jobs"][job]["steps"][0]
        _require(raw == {"name": "Checkout", "uses": "actions/checkout@3d3c42e5aac5ba805825da76410c181273ba90b1",
                         "with": {"fetch-depth": "0", "persist-credentials": "false"}}, "residual_input_checkout_profile", job)
        equations[_step_id(job, 1)] = ((), ())
    return {"locators": {role: path for path, role in names.items()},
...
def _verify_source_residual_input_equations(
    ...
    """Reject wrong supplied relations even when both constructors agree."""
    facts = _source_residual_input_expectations(workflow, sources)
    states = {s["state_id"].removeprefix("state:step5c:"): s for s in plan["state_templates"]}
    all_steps = [s for job in plan["jobs"] for s in job["steps"]]
    steps = {s["occurrence_id"]: s for s in all_steps}
    _require(len(states) == len(plan["state_templates"]) and len(steps) == len(all_steps),
             "residual_input_duplicate_identity")
    for role, path in facts["locators"].items():
        _require(role in states, "residual_input_role_missing", role)
        row = states[role]
        _require(row["path_or_uri"] == path, "residual_input_locator_mismatch", role)
        _require(row["producer_occurrence_id"] == facts["origins"][role], "residual_input_origin_mismatch", role)
        authority = role not in ("release-grade-junit", "release-grade-sarif")
        mutation = "final_status" if role == "final-status" else "none"
        _require(row["required"] is True and row["content_requirement"] == "exact_digest"
                 and row["authority_bearing"] is authority and row["mutation_class"] == mutation,
                 "residual_input_duty_mismatch", role)
    for oid, eq in facts["steps"].items():
        _require(oid in steps, "residual_input_step_missing", oid)
        for field, direction in (("input_state_ids", "inputs"), ("output_state_ids", "outputs")):
            _require(steps[oid][field] == sorted("state:step5c:" + role for role in eq[direction]),
                     "residual_input_step_io_mismatch", oid + ":" + direction)
        _require(sorted(row["state_id"] for row in states.values() if oid in row["required_consumer_occurrence_ids"])
                 == steps[oid]["input_state_ids"], "residual_input_reverse_mismatch", oid)
    source = states["workflow-source"]
    # L6 really hashes the workflow while building its envelope. Preserve that
    # completed read; removing checkout-as-read must not erase this consumer.
    declared = _source_llamaguard_attestation_expectations(workflow, sources)
    expected_reads = sorted(oid for oid, eq in declared["steps"].items() if "workflow-source" in eq["inputs"])
    _require(source["required_consumer_occurrence_ids"] == expected_reads
             and sorted(step["occurrence_id"] for step in all_steps if source["state_id"] in step["input_state_ids"])
             == expected_reads, "residual_input_source_is_not_read_receipt")
    # A structural cross-check is additional, never a source oracle: it cannot
    # discover a relation omitted from both sides of the graph.
    by_id = {s["state_id"]: s for s in states.values()}
    for oid, step in steps.items():
        _require(step["input_state_ids"] == sorted(row["state_id"] for row in by_id.values()
...
def _build_states(
    ...
    report_publication = _source_report_publication_expectations(source_workflow, source_by_path)
    ...
    _install_residual_input_projection(states, step_by_key, _source_residual_input_expectations(source_workflow, source_by_path))
...
def _assemble_expected_plan_from_sources(
...
def _local_r2_checked_sources(
    source_index_raw: bytes, source_members: dict[str, bytes], *,
    expected_source_tree: str, expected_source_index_sha256: str,
) -> dict[str, GitObject]:
    """Independent finite index/tree and exact required-byte verification; no Git writes."""
    ...
    _require(type(source_index_raw) is bytes and 0 < len(source_index_raw) <= 8 * 1024 * 1024,
             'r2_source_inventory_invalid')
    _require(hashlib.sha256(source_index_raw).hexdigest() == expected_source_index_sha256,
             'r2_source_index_mismatch')
    index = _strict_json_object(source_index_raw, label='local_source_index')
    _require(_canonical_json_bytes(index) == source_index_raw, 'r2_source_index_not_canonical')
    _require(set(index) == {'source_kind', 'files'} and index['source_kind'] == 'uncommitted_git_tree',
             'r2_source_inventory_invalid')
    rows = index['files']
    _require(type(rows) is list and 0 < len(rows) <= 4096, 'r2_source_inventory_invalid')
    table: dict[str, dict[str, Any]] = {}
    directories: dict[str, dict[str, tuple[str, str]]] = {'': {}}
    total = 0
    for row in rows:
        _require(type(row) is dict and set(row) == {'path', 'git_mode', 'git_blob_sha1', 'sha256', 'size_bytes'},
                 'r2_source_entry_invalid')
        path = row['path']
        _require(type(path) is str and 0 < len(path.encode('utf-8')) <= 1024
                 and '\\' not in path and '\0' not in path
                 and len(path.split('/')) <= 64
                 and all(p not in {'', '.', '..', '.git'} for p in path.split('/')), 'r2_source_path_invalid')
        _require(path not in table, 'r2_source_path_duplicate')
        _require(type(row['git_mode']) is str and row['git_mode'] in {'100644', '100755'} and type(row['size_bytes']) is int
                 and 0 <= row['size_bytes'] <= 8 * 1024 * 1024, 'r2_source_entry_invalid')
        _require(type(row['git_blob_sha1']) is str and re.fullmatch(r'[0-9a-f]{40}', row['git_blob_sha1'])
                 is not None and type(row['sha256']) is str and re.fullmatch(r'[0-9a-f]{64}', row['sha256'])
                 is not None, 'r2_source_entry_invalid')
        total += row['size_bytes']
        _require(total <= 512 * 1024 * 1024, 'r2_source_budget_exceeded')
        table[path] = row
        parts = path.split('/')
        for length in range(1, len(parts)):
            parent = '/'.join(parts[:length - 1]); name = parts[length - 1]
            previous = directories.setdefault(parent, {}).get(name)
            _require(previous is None or previous[0] == '40000', 'r2_source_path_collision')
            directories[parent][name] = ('40000', '/'.join(parts[:length]))
            directories.setdefault('/'.join(parts[:length]), {})
        parent = '/'.join(parts[:-1]); name = parts[-1]
        _require(name not in directories[parent], 'r2_source_path_collision')
        directories[parent][name] = (row['git_mode'], row['git_blob_sha1'])
    _require([r['path'] for r in rows] == sorted(table), 'r2_source_index_order_mismatch')
    hashes: dict[str, str] = {}
    for parent in sorted(directories, key=lambda p: (len(p.split('/')) if p else 0, p), reverse=True):
        entries = []
        for name, (mode, value) in directories[parent].items():
            oid = hashes[value] if mode == '40000' else value
            entries.append((name.encode('utf-8') + (b'/' if mode == '40000' else b''),
    ...
    for role, path in _LOCAL_R2_SOURCE_ROLES:
        ...
        _require(type(raw) is bytes and 0 < len(raw) == row['size_bytes']
                 ...
                 and _sha1_git_blob(raw) == row['git_blob_sha1'], 'r2_source_bytes_mismatch', path)
        result[path] = GitObject(role, path, expected_source_tree, row['git_mode'], row['git_blob_sha1'], raw)
    _require(Path(__file__).read_bytes() == result[PLAN_CHECKER_PATH].data,
             'r2_executed_checker_source_mismatch')
    _check_reviewed_source_pins(result)
    return result


@_yaml_parse_scope()
def _check_local_r2_plan(
    ...
    """Reconstruct the local prelaunch graph independently; never declare I/E complete."""
    _require(type(expected_plan_sha256) is str and re.fullmatch(r'[0-9a-f]{64}', expected_plan_sha256)
             is not None, 'r2_expected_plan_digest_invalid')
    _require(type(plan_raw) is bytes and 0 < len(plan_raw) <= MAX_PLAN_BYTES, 'r2_plan_bytes_invalid')
    _require(hashlib.sha256(plan_raw).hexdigest() == expected_plan_sha256, 'r2_plan_digest_mismatch')
    plan = _strict_json_object(plan_raw, label='local_r2_plan')
    _require(_canonical_json_bytes(plan) == plan_raw, 'r2_plan_not_canonical')
    sources = _local_r2_checked_sources(
        source_index_raw, source_members, expected_source_tree=expected_source_tree,
        expected_source_index_sha256=expected_source_index_sha256,
    )
    binding = plan.get('local_requirement_binding')
    _verify_local_r2_requirement_binding(binding, sources[SCHEMA_PATH].data,
                                         expected_schema_sha256=sources[SCHEMA_PATH].sha256)
    ...
    expected['record_status'] = 'local_candidate'
    identity = expected['plan_identity']
    del identity['source_commit']
    identity['source_identity'] = {
        'kind': 'uncommitted_git_tree', 'git_tree_sha1': expected_source_tree,
        'source_index_sha256': expected_source_index_sha256,
    }
    for key in ('builder', 'independent_checker'):
        identity[key]['source_revision_kind'] = 'uncommitted_git_tree'
    for row in expected['source_inventory']:
        row['revision_kind'] = 'uncommitted_git_tree'
    expected['local_requirement_binding'] = deepcopy(binding)
    expected['local_boundary'] = {
        'dispatch_authorized': False, 'R2_activated': False,
        'completion_evaluated': False, 'reference_acquired': False,
    }
    _require(_canonical_json_bytes(expected) == plan_raw, 'r2_plan_reconstruction_mismatch')
    _check_sorted_and_unique(plan)
    workflow = _parse_yaml_document(sources[SUBJECT_WORKFLOW_PATH].data, label=SUBJECT_WORKFLOW_PATH)
    _verify_source_ledger_equations(plan, workflow)
    for verify_equations in (
        _verify_source_recorded_equations, _verify_source_package_equations,
        _verify_source_required_argument_equations, _verify_source_bundle_equations,
        _verify_source_provenance_equations, _verify_source_baseline_floor_equations,
        _verify_source_preattest_preservation_equations, _verify_source_llamaguard_preservation_equations,
        _verify_source_llamaguard_attestation_equations, _verify_source_llamaguard_production_equations,
        _verify_source_pre_attestation_postcondition_equations, _verify_source_final_artifact_postcondition_equations,
        _verify_source_recorded_publication_equations, _verify_source_authority_publication_equations,
        _verify_source_report_publication_equations, _verify_source_residual_input_equations,
    ):
        verify_equations(plan, workflow, sources)
    return {
        'schema_version': 'pulsemech_step5c_local_r2_plan_check_v0',
        'record_status': 'local_candidate', 'ok': True, 'errors': [],
        'source_identity': deepcopy(identity['source_identity']),
        'plan': {'sha256': expected_plan_sha256, 'size_bytes': len(plan_raw),
                 'byte_identical_to_independent_reconstruction': True},
        'checker': deepcopy(identity['independent_checker']),
        'local_requirement_binding': deepcopy(binding), 'counts': _plan_counts(plan),
