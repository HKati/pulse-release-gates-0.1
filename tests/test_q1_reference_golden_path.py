#!/usr/bin/env python3
"""
Golden-path smoke test for the checked-in Q1 reference artefacts.
...
It locks down:
1. the checked-in summary example conforms to the shared Q1 summary schema
2. the runner reproduces the checked-in summary exactly from checked-in inputs
"""
    ...
    for start in starts:
        for candidate in (start, *start.parents):
            ...
            if (
                ...
                candidate / "examples" / "q1_groundedness_summary.example.json"
            ).is_file():
                return candidate

    raise RuntimeError(
        "Could not locate repo root containing the Q1 runner, schema, and "
        "checked-in Q1 reference artefacts"
    )
...
EXAMPLE = ROOT / "examples" / "q1_groundedness_summary.example.json"

MANIFEST_REL = "examples/q1_reference_input_manifest.json"
LABELS_REL = "examples/q1_reference_labels.pass_120.jsonl"
RUN_ID = "q1-ref-2026-03-09T20:00:00Z"
CREATED_UTC = "2026-03-09T20:00:00Z"
TOOL = "PULSE_q1_reference"
TOOL_VERSION = "0.1.0-dev"
GIT_SHA = "example-q1-ref"
...
def _run_runner() -> tuple[subprocess.CompletedProcess[str], dict | None]:
    with tempfile.TemporaryDirectory() as td:
        out_path = Path(td) / "q1_reference_summary.json"
...
def test_checked_in_q1_summary_example_matches_schema() -> None:
    schema = _load_schema()
    example = _load_json(EXAMPLE)

    jsonschema.validate(instance=example, schema=schema)

    assert example["spec_id"] == "q1_groundedness_v0"
    ...
    assert example["provenance"]["labels_jsonl"] == LABELS_REL
    assert example["provenance"]["tool"] == TOOL


def test_runner_reproduces_checked_in_q1_summary_example() -> None:
    schema = _load_schema()
    ...
    if actual != expected:
        raise AssertionError(
            "Checked-in Q1 summary example drifted from current runner output.\n\n"
...
def main() -> int:
    try:
        test_checked_in_q1_summary_example_matches_schema()
        test_runner_reproduces_checked_in_q1_summary_example()
    except AssertionError as e:
        print(f"ERROR: {e}")
        return 1

    print("OK: Q1 reference golden-path smoke passed")
    return 0
