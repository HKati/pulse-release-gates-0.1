...
Each route has a different completion signal.
...
| Evidence-admission finding | A problem in how candidate evidence becomes recorded release evidence | Release-relevant when the evidence is required by declared policy | Explicit admission rule, status representation, required gate materialization, fail-closed enforcement |
...
| Traceability / audit finding | A reconstruction issue across evidence, policy, evaluator, run identity, manifest, digest, or audit bundle | Reconstruction relevant | Trace carrier fix, manifest row, digest binding, audit bundle update |
...
| Supply-chain hardening finding | Artifact integrity, provenance, attestations, signing, or workflow integrity needs strengthening | Supporting assurance relevant | Signing / attestation / provenance / workflow-hardening implementation |
...
| Category framing finding | PULSE is read as governance, dashboard, MLOps, runtime guardrail, supply-chain framework, adoption project, or maturity project | Category-boundary relevant | Terminology boundary fix, review entrypoint wording, risk-map route |
...
| PULSE is read primarily as MLOps platform | Category framing / integration-context finding | External review reading-frame routing | Route MLOps comparisons as integration or candidate-policy findings | No direct release effect |
| PULSE is read primarily as runtime guardrail | Category framing / boundary-context finding | Release boundary vs runtime boundary split | Keep runtime comparison as contextual boundary finding | No direct release effect |
| PULSE is read primarily as supply-chain framework | Supply-chain hardening finding | Hardening plan treats signing, SLSA, Sigstore, in-toto, and provenance as support layers | Add support layers only as hardening, not identity | Release effect only if declared as required gates |
| External review approval is treated as release permission | External-review admission finding | External review entrypoint state model | Route review output through evidence admission before release effect | Release effect only after recorded evidence, declared policy, materialized gate, and CI enforcement |
| Public Pages or badge visibility is treated as authority | Reader-surface / publication-surface finding | Public surfaces are reader / publication carriers | Maintain publication snapshot and reader-surface wording | Public visibility alone has no release effect |
...
| Diagnostic or shadow output is treated as authority | Diagnostic / shadow separation finding | Optional and shadow surfaces are non-authorizing by default | Keep shadow non-interference tests and boundary wording | Release effect only after explicit promotion into required gates |
| Green CI outside the workflow-effective required gate set is treated as authority | CI-surface finding | PULSEmech requires declared policy and materialized required gates | Keep required-gate CI enforcement explicit | Release effect only through declared-policy CI allow/block path |
| Release semantics can silently drift | Authority-path / CI hardening finding | Hardening plan CR-01 and CR-02 | Add or maintain contract tests and required CI checks | Direct release-authority risk until locked |
...
| Dependency truth is split | Reproducibility / dependency finding | Hardening plan CR-03 | Move toward single dependency truth and clean-install smoke | Hardening risk, not PULSEmech definition |
| Script-heavy import structure is fragile | Packaging / maintainability finding | Hardening plan CR-04 | Package layout and compatibility wrappers | Hardening risk, not PULSEmech definition |
...
| Public/private artifact boundary is unclear | Publication / security hardening finding | Hardening plan publication-surface clarity | Add public/private artifact classification and sanitization checks | Hardening and trust risk |
...
| Branch protection is incomplete | CI / repository hardening finding | Hardening plan CI hardening phase | Align required checks and branch protection with release core | Infrastructure risk around release path |
...
| Maintainer concentration is used as mechanism criticism | Sustainability finding | Reviewable mechanics and docs reduce bus-factor | Add contributor path, review packet, reproducibility exercises | Sustainability risk, not mechanism invalidation |
| Enterprise maturity score is used as PULSE validity | Maturity / adoption-context finding | Hardening plan separates repository maturity from PULSE definition | Route as roadmap or hardening observation | No direct release-authority effect |
...
|---|---|---|
| `README.md` | Curated identity, release-authority map, start path, authority boundary | Category framing, reader-surface confusion, external-review substitution |
| `HARDENING_PLAN.md` | Repository and release-grade infrastructure hardening sequence | CI hardening, dependency truth, packaging, external evidence misunderstanding, public-surface ambiguity |
| `docs/PULSE_EXTERNAL_REVIEW_ENTRYPOINT_v0.md` | External review operating route and review-output state model | Review discoverability, review-output routing, finding classification |
| `docs/PULSE_TERMINOLOGY_RISK_REGISTER_v0.md` | Category and terminology boundary control | Governance, dashboard, MLOps, runtime, supply-chain, adoption, maturity misclassification |
| `docs/release_authority_manifest_v0.md` | Contract for release authority manifest as audit / trace carrier | Traceability, audit reconstruction, non-authorizing manifest boundary |
| `docs/STATUS_CONTRACT.md` | Status artifact contract | Status representation and gate semantics |
| `docs/status_json.md` | Reader guidance for `status.json` | Status reading, reader interpretation |
| `docs/RUNBOOK.md` | Operational triage | Blocked release diagnosis |
| `docs/EXTERNAL_DETECTORS.md` | External detector policy and modes | External evidence admission and advisory/gating distinction |
| `docs/external_detector_summaries.md` | External detector summary fold-in path | Evidence presence vs aggregate pass distinction |
| `docs/quality_ledger.md` | Quality Ledger reader-surface contract | Reader-surface boundary |
| `docs/WORKFLOW_MAP.md` | Workflow role map | Workflow authority split, diagnostic vs release lane separation |

## Hardening route by class

### Authority-path route
...
### Evidence-admission route

Use this route when a finding concerns candidate evidence, external review
output, detector output, summaries, or review artifacts.
...
integrity, workflow integrity, artifact provenance, or publication trust.
...
critique into one of these routes:
...
A critique is useful when it is routed.
