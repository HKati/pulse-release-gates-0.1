...
If the release decision is `FAIL` and an override is accepted, the system
records two facts:
...
A break-glass artifact must contain at minimum:
...
A valid accepted override requires a review block.

The review block must record:
...
Shadow outputs may inform human review, but they do not authorize break-glass
by themselves.
...
The artifact chain should make it possible to reconstruct why the normal
release path did not pass and why an exception was accepted.
