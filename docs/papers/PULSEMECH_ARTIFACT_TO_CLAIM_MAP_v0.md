...
| Verification status | Meaning | Manuscript promotion |
|---|---|---|
| `to verify before manuscript` | Initial state. Artifact references, authority role, and boundary wording still require review. | Not promotable |
| `verified for manuscript` | Artifact references, authority role, and non-claim boundary have been checked for manuscript use. | Promotable |
| `accepted limitation` | Explicit limitation / boundary claim has been checked as a valid manuscript limitation. Repository artifact support is optional when the statement is purely a non-claim. | Promotable as limitation |
| `accepted future-work` | Explicit future-work claim has been checked and remains clearly marked as future work. Repository artifact support is optional when the statement is not written as current capability. | Promotable as future work |

## Authority / role status vocabulary

| Authority / role status | Meaning |
|---|---|
...
| `normative support` | Stabilizes or supports interpretation of normative inputs, but does not create release authority by itself. |
...
| `non-normative fixture` | Provides controlled test or reference candidate state, but does not create release authority. |
| `non-normative checker` | Checks artifact shape, consistency, or reconstruction readiness, but does not create release authority. |
| `non-normative packet-preparation` | Prepares packet artifacts for reconstruction or later verification, but does not create release authority. |
| `non-normative regression` | Detects drift in generated or recorded artifacts, but does not create release authority. |
| `non-normative publication` | Covers publication or public-surface integrity checks; publication surfaces do not create release authority. |
| `non-normative field principle` | Explains or preserves field interpretation without replacing the normative PULSEmech path. |
| `paper-planning only` | Organizes future manuscript work and does not change repository behavior. |
| `boundary-claim` | States a limitation or non-claim. |
| `future-work` | Describes later work and must not be written as current capability. |

The `Authority / role status` column must use only values from this vocabulary.

If multiple statuses are needed, each status must be present in this vocabulary.

Claim-support terms such as `artifact-supported`, `test-supported`, `fixture`, `checker`, `regression`, or `field-principle` must not be introduced as ad hoc authority labels.

Support type belongs in the evidence role, verification status, or artifact evidence columns.

## Artifact-to-claim table

| Claim ID | Paper claim | Artifact evidence | Evidence role | Authority / role status | Boundary / non-claim | Verification status |
|---|---|---|---|---|---|---|
| `C01_RELEASE_AUTHORITY_IDENTITY` | PULSE is an artifact-bound release-authority mechanism for AI applications and AI-enabled systems. | `README.md`; `docs/papers/PULSEMECH_CS_SE_PAPER_SKELETON_v0.md` | Defines public identity and paper scope. | paper-planning only | Does not claim PULSE is a generic governance framework or dashboard. | to verify before manuscript |
...
| `C03_PRE_RELEASE_MEASUREMENT` | PULSE evaluates release permission before release effects propagate. | `README.md`; `docs/PULSE_RELATIONAL_EVIDENCE_FIELD_PRINCIPLE_v0.md`; `docs/papers/PULSEMECH_CS_SE_PAPER_SKELETON_v0.md` | Establishes pre-release measurement boundary. | normative path | Does not explain post-event outcomes as release authority. | to verify before manuscript |
...
| `C06_GATE_REGISTRY` | Gate meaning is stabilized through the gate registry. | `pulse_gate_registry_v0.yml` | Supports stable gate identifiers and semantics. | normative support | Does not replace declared policy or CI enforcement. | to verify before manuscript |
| `C07_MATERIALIZED_REQUIRED_GATES` | Required and release_required gates can be materialized from declared policy. | `tools/policy_to_require_args.py`; `gates/materialized_gate_sets.json` in generated packets; `scripts/build_pulse_ref_schema_aligned_pass_fixture_packet_v0.py`; `scripts/check_pulse_ref_schema_aligned_packet_v0.py` | Connects declared policy to concrete enforced gate set. | normative input | Does not allow hard-coded promotion of gates outside policy. | to verify before manuscript |
...
| `C10_AUTHORITY_BOUNDARY` | PULSE separates normative release-authority surfaces from trace, audit, reader, diagnostic, and packet-preparation surfaces. | `README.md`; `docs/OPTIONAL_LAYERS.md`; `docs/SPACE_RELATION_MAP_v0.md`; `docs/papers/PULSEMECH_CS_SE_PAPER_SKELETON_v0.md` | Prevents surface-role collapse. | paper-planning only | Does not promote dashboards, ledgers, badges, manifests, or packets into authority by existence. | to verify before manuscript |
...
| `C22_RESULT_NOT_MECHANISM` | A release outcome is not equivalent to the release mechanism. | `docs/papers/PULSEMECH_CS_SE_PAPER_SKELETON_v0.md`; workshop principle; PULSEmech tuple | Supports paper framing around pre-release relation measurement. | paper-planning only | Should be written as mechanism framing, not philosophical proof. | to verify before manuscript |
...
| `C26_NOT_FRAMEWORK_IDENTITY` | The paper should not identify PULSE as a generic framework. | README front-door guard; `tests/test_readme_release_authority_category_signal_v0.py`; paper skeleton terminology boundary | Protects category identity. | paper-planning only | May use generic terms only when not defining identity. | to verify before manuscript |
...
| `C28_AI_ASSISTED_DRAFTING_DISCLOSURE` | AI-assisted drafting and technical editing should be disclosed in the manuscript. | paper skeleton disclosure placeholder | Publication-process support. | paper-planning only | AI tools are not authors; human author remains responsible. | to verify before manuscript |
| `C29_REPRODUCIBILITY_SPINE` | The paper should present reproducibility through artifacts, validators, tests, and regression rather than unsupported narrative. | PULSE-REF builder, validator, regression, CI tests | Organizes validation section. | paper-planning only | Does not imply external replication has already occurred. | to verify before manuscript |
...
2. it has at least one repository artifact or an explicit limitation / future-work classification;
3. its authority / role status is identified using only values from the authority / role status vocabulary;
4. its non-claim boundary is stated;
5. its verification status is resolved as one of:
   - `verified for manuscript`;
   - `accepted limitation`;
   - `accepted future-work`;
6. it does not inflate a non-normative surface into release authority.

Claims with verification status `to verify before manuscript` must not be promoted into manuscript prose.

## Claim rejection rule

Reject or rewrite the claim when:

- it has no repository artifact and is not explicitly classified as an accepted limitation or accepted future-work claim;
- its verification status is still `to verify before manuscript`;
- it uses an authority / role status not defined in the authority / role status vocabulary;
...
## Validation checks for this map

Before merging changes to this map, run:

```bash
grep -n "Verification status vocabulary" docs/papers/PULSEMECH_ARTIFACT_TO_CLAIM_MAP_v0.md
grep -n "Claims with verification status \`to verify before manuscript\` must not be promoted" docs/papers/PULSEMECH_ARTIFACT_TO_CLAIM_MAP_v0.md
grep -n "Authority / role status vocabulary" docs/papers/PULSEMECH_ARTIFACT_TO_CLAIM_MAP_v0.md
grep -n "normative support" docs/papers/PULSEMECH_ARTIFACT_TO_CLAIM_MAP_v0.md
grep -n "non-normative regression" docs/papers/PULSEMECH_ARTIFACT_TO_CLAIM_MAP_v0.md
grep -n "accepted limitation or accepted future-work" docs/papers/PULSEMECH_ARTIFACT_TO_CLAIM_MAP_v0.md
```

Expected result:

- verification status vocabulary is present;
- `to verify before manuscript` is explicitly non-promotable;
- authority / role status vocabulary is present;
- `normative support` is defined;
- `non-normative regression` is defined;
- limitation / future-work exceptions are preserved.
