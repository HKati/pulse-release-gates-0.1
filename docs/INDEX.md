...
- Detailed compute binding, current-run automation and bounded observed execution:
  ...
  **Current implementation and completed bounded proof.** Records the completed
  ...
  The
  [Step 5B system-level record](../PULSEMECH_TECHNICAL_OVERVIEW.md#step-5b-bounded-observed-reference)
  binds implementation PR #2876, owner-dispatched run `34402387728`, artifact
  `10123984621`, preservation PR #2877 and independent source-bound replay.
  Within `six_declared_direct_processes_and_their_bound_io`, I, E and R are
...
historical Step 4A capture, bounded Step 4B producer/preservation, accepted
Step 5A runtime-bound report/relation processing and the completed Step 5B
bounded checker/consumer reference. It separates permanent regressions,
historical reconstruction, the actual owner-dispatched Step 5B acquisition,
exact preservation, independent replay and their respective review limits.
Step 5B closes one finite six-process observation boundary; whole-runtime
live/current-run observation, complete Step 5 closure, resource measurement
and policy promotion remain later work. Future operational current-run source
capture must precede its authority decision.
...
4. [Compute-binding workstream record](compute/PULSEMECH_COMPUTE_BINDING_AND_TRANSITION_EFFICIENCY_DESIGN_v0.md)
   Detailed compute contracts, completed Step 3G automation, exact historical
   Step 4A capture, bounded Step 4B preservation, accepted Step 5A processing,
   completed Step 5B observation and replay, and the remaining whole-runtime
...
- [compute/PULSEMECH_COMPUTE_BINDING_AND_TRANSITION_EFFICIENCY_DESIGN_v0.md](compute/PULSEMECH_COMPUTE_BINDING_AND_TRANSITION_EFFICIENCY_DESIGN_v0.md) — **Current implementation and completed bounded proof.** Detailed compute contracts, completed Step 3G automation and regressions, exact historical Step 4A capture, bounded Step 4B preservation, accepted Step 5A runtime-bound construction and the completed Step 5B owner-dispatched six-process reference with exact preservation and source-bound replay; whole-runtime observation, complete Step 5 closure and later measurement remain separate.  
...
| Compute binding, current-run automation, historical capture and bounded observation | Maps executed compute to transition roles and records completed Step 3G automation, Step 4A capture, bounded historical Step 4B packet production/preservation, accepted Step 5A runtime-bound processing and the completed Step 5B six-process checker/consumer reference. Separates historical and actual bounded evidence, source-bound replay and review limits. Keeps whole-runtime observation, complete Step 5 closure, measurement and future pre-decision operation distinct; no compute-gate activation or release-authority change. | [compute/PULSEMECH_COMPUTE_BINDING_AND_TRANSITION_EFFICIENCY_DESIGN_v0.md](compute/PULSEMECH_COMPUTE_BINDING_AND_TRANSITION_EFFICIENCY_DESIGN_v0.md), [PULSEMECH_TECHNICAL_OVERVIEW.md](../PULSEMECH_TECHNICAL_OVERVIEW.md) |
...
- [Compute-binding design and implementation-state record](compute/PULSEMECH_COMPUTE_BINDING_AND_TRANSITION_EFFICIENCY_DESIGN_v0.md) — Detailed canonical record of completed Step 3G automation, historical Step 4A capture, bounded Step 4B production/preservation, accepted Step 5A at PR #2872 / `5afb4404719fb04d3ee32a007cdbb8b47c220118`, and completed Step 5B through implementation PR #2876 / `c32508f8afb58381225fec0b426b85b00e32fe11`, owner-dispatched run `34402387728`, artifact `10123984621`, and preservation PR #2877 / `12b42736a9f8a1da16e3659a2d2099058207129b`. Historical Step 4B packet preservation remains #2868 / `55c6180dfbfd4055a558cf6b3e3883461a423f86`, with its producer separately fixed at #2867.
...
<a id="open-implementation-workstream"></a>

### Completed current-run automation — Step 3G

[PR #2789 — current-run export expectation builder](https://github.com/HKati/pulse-release-gates-0.1/pull/2789)
is a merged implementation milestone, not an open workstream. Its subsequent
regression and the current-run components are recorded in the detailed
workstream document. The former open-workstream anchor is retained here for
navigation continuity.

- [Current-run expectation builder](../tools/build_pulsemech_compute_current_run_export_expectation_v0.py) and [permanent builder regression](../tests/test_build_pulsemech_compute_current_run_export_expectation_v0.py) — Implemented, hardened and regression-proven.
- [Finalized export-carrier loader](../tools/load_pulsemech_compute_current_run_export_carrier_v0.py) and [carrier regression](../tests/test_load_pulsemech_compute_current_run_export_carrier_v0.py) — Exact finalized carrier and protected-control-plane binding.
- [Current-run subject-input wrapper](../tools/build_pulsemech_compute_subject_input_packet_current_run_v0.py) and [wrapper regression](../tests/test_build_pulsemech_compute_subject_input_packet_current_run_v0.py) — Current-run production through the existing reusable producer core.
- [Step 3F export candidate workflow](../.github/workflows/pulsemech_compute_current_run_export_candidate.yml) and [workflow regression](../tests/test_pulsemech_compute_current_run_export_candidate_workflow_v0.py) — Manual, non-active candidate export.
- [Step 3G candidate-bundle intake](../tools/load_pulsemech_compute_current_run_export_candidate_bundle_v0.py) and [intake regression](../tests/test_load_pulsemech_compute_current_run_export_candidate_bundle_v0.py) — Independent checksum-closed candidate-bundle verification.
- [Artifact-observed proof builder](../tools/build_pulsemech_compute_current_run_artifact_observed_proof_v0.py) and [proof-builder regression](../tests/test_build_pulsemech_compute_current_run_artifact_observed_proof_v0.py) — Uses the existing analyzer bridge and reusable core, preserving incomplete relation state.
- [Step 3G artifact-observed candidate workflow](../.github/workflows/pulsemech_compute_current_run_artifact_observed_candidate.yml) and [workflow regression](../tests/test_pulsemech_compute_current_run_artifact_observed_candidate_workflow_v0.py) — Manual, `diagnostic_shadow` and non-active orchestration.

The implementation and permanent regression boundary is complete through
[PR #2826](https://github.com/HKati/pulse-release-gates-0.1/pull/2826), merge
`9bf7fab95dbcc3532238723d0cf76500263106f5`.

The source subject, protected control plane, finalized carrier, expectation,
observed subject-input packet, artifact-observed report, planned-observed
relation and separate folded candidate status remain exactly bound. Actual
false, missing, partial, ambiguous and unresolved values remain visible.

This records implemented construction capability and permanent regression
proof. It does not claim a public manually dispatched Step 3F/3G proof instance.
The separate Step 4A acquisition below is not such an instance.

```text
Step 3G implementation and permanent regressions:
complete

Step 3F/3G workflows:
manual, candidate-only, non-active and pre-authority

source status.json mutation:
none
...
### Completed historical producer-input capture — Step 4A

**Current implementation and completed bounded proof.** The detailed
[completed Step 4A record](compute/PULSEMECH_COMPUTE_BINDING_AND_TRANSITION_EFFICIENCY_DESIGN_v0.md#completed-step-4a--exact-post-run-producer-input-capture)
contains the five-member identity table, original acquisition facts, offline
replay, manifest reconstruction, targeted negative evidence and source limits.
The [Technical Overview capture section](../PULSEMECH_TECHNICAL_OVERVIEW.md#15i-completed-historical-post-run-producer-input-capture)
records the same completed boundary at system level.

- [Capture-manifest schema](../schemas/pulsemech_compute_post_run_producer_input_capture_manifest_v0.schema.json) and [normative capture contract](../contracts/pulsemech_compute_post_run_producer_input_capture_v0.json) — Exact producer-input, request, inventory, temporal and authority boundaries.
- [Networked capture tool](../tools/capture_pulsemech_compute_post_run_producer_input_v0.py) — Preserves the attempt-specific platform-response entity-body bytes and selected exchange metadata.
- [Independent offline validator](../tools/check_pulsemech_compute_post_run_producer_input_capture_v0.py) — Network-free reconstruction and verification; neither imports the capture implementation nor trusts its verdict.
- [Fixed-subject acquisition workflow](../.github/workflows/pulsemech_compute_post_run_producer_input_capture_v0.yml) — Manual capture, independent validation and mode-preserving TAR transport; no release-authority effect.
- [Contract regression](../tests/test_pulsemech_compute_post_run_producer_input_capture_contract_v0.py), [producer regression](../tests/test_capture_pulsemech_compute_post_run_producer_input_v0.py) and [offline-validator/TAR regression](../tests/test_check_pulsemech_compute_post_run_producer_input_capture_v0.py) — Permanent implementation and fixture coverage registered in [ci/tools-tests.list](../ci/tools-tests.list).
- [Preserved #6066 capture directory](../preservation/pulse_ci_6066/post_run_producer_input_capture_v0/) and [canonical capture manifest](../preservation/pulse_ci_6066/post_run_producer_input_capture_v0/pulsemech_compute_post_run_producer_input_capture_manifest_6066_v0.json) — The five exact observed data members, not a regenerated response or an example fixture.

The historical subject is PULSE CI #6066, run `29249887581`, attempt `1`,
source `46b639706e23f80fe296a8893be18e2b5ab21f7e`. The separate
[acquisition run `33986538130`](https://github.com/HKati/pulse-release-gates-0.1/actions/runs/33986538130),
attempt `1`, used source `22d14088ae21f84d94c6a6951c0f70ab1bdf0895`.
The five data members were preserved through
[PR #2861](https://github.com/HKati/pulse-release-gates-0.1/pull/2861), merge
`7444c12c3c9a86591f0aa7f5cef759ec55e6f9e9`.

The recorded post-merge review verified exact merged bytes, two independent
offline replays with identical diagnostics, two exact manifest reconstructions
and ten rejected disposable-copy probes across nine mutation categories.
Permanent CI coverage of implementation and fixtures remains distinct from
those review executions against the actual committed capture. The detailed
record separates cloud execution, separately inspected GitHub CI and supplied
original-carrier identities; this index is not a new verification execution.

```text
historical capture subject:
post-run platform-response snapshot observed at capture time

preserved input:
two exact raw responses, two exchange records and one manifest

page / unique job / step counts:
1 / 8 / 171

same_run_release_authority_eligible:
false

active_gate_eligible:
false
...
The capture is not the original runtime network stream, a runtime-observation
packet, a transition measurement, a resource measurement or a retroactive
release-authority input. It does not modify the earlier release-grade #6066
...
### Completed runtime-bound report and relation — Step 5A

**Current implementation and completed bounded handoff.**
[PR #2872](https://github.com/HKati/pulse-release-gates-0.1/pull/2872), merge
`5afb4404719fb04d3ee32a007cdbb8b47c220118`, adds the runtime-bound path through the
existing immutable bridge, analyzer core, report/relation validators and separate
candidate materializer. The artifact baseline and runtime evidence stay distinct.

- [Detailed Step 5A implementation, evidence and remaining boundary](compute/PULSEMECH_COMPUTE_BINDING_AND_TRANSITION_EFFICIENCY_DESIGN_v0.md#completed-step-5a--runtime-bound-report-and-relation).
- [System-level Step 5A result](../PULSEMECH_TECHNICAL_OVERVIEW.md#15k-runtime-bound-report-and-relation--step-5a).
- [Closed work order and bounded acceptance](https://github.com/HKati/pulse-release-gates-0.1/issues/2870#issuecomment-5577131873).
- [Post-merge Tools smoke execution](https://github.com/HKati/pulse-release-gates-0.1/actions/runs/34170196053/job/101888978377).

Source review and GitHub execution are separate evidence: the cloud review's
historical-source failures remain recorded; the merge-bound CI log supplies the
successful historical-chain execution and 418 affected-suite passes. The unchanged
manifest has 153 unique program paths after filtering comments and blank lines.
The prior 157-text-entry review count is corrected in the acceptance record.

The actual #6066 packet remains unchanged and partial. Complete synthetic extent
does not establish observed terminal closure. The full Step 5 observed proof,
live/current-run observation, resource measurement and policy promotion remain
...
**Current implementation and completed bounded proof.**

The
[bounded execution contract](compute/PULSEMECH_COMPUTE_BOUNDED_EXECUTION_CONTRACT_v0.md)
and [evidence schema](../schemas/pulsemech_compute_bounded_execution_evidence_v0.schema.json)
define the finite checker/consumer reference under work order
[#2875](https://github.com/HKati/pulse-release-gates-0.1/issues/2875).
...
→ owner-dispatched workflow run 34402387728 / attempt 1
→ job 102637163024
→ artifact 10123984621

→ PR #2877
12b42736a9f8a1da16e3659a2d2099058207129b
tree b44c9f7cfa24c0dafbc8c38973fbbca37295f7d1
...
- [System-level Step 5B result](../PULSEMECH_TECHNICAL_OVERVIEW.md#step-5b-bounded-observed-reference).
- [Implementation PR #2876](https://github.com/HKati/pulse-release-gates-0.1/pull/2876).
- [Owner-dispatched reference run 34402387728](https://github.com/HKati/pulse-release-gates-0.1/actions/runs/34402387728).
- [Preservation PR #2877](https://github.com/HKati/pulse-release-gates-0.1/pull/2877).
- [Exact preservation and replay record](../preservation/compute_bounded_execution_reference_v0/run_34402387728_attempt_1/README.md).
- [Bounded acceptance and canonical-sync authorization](https://github.com/HKati/pulse-release-gates-0.1/issues/2875#issuecomment-5609641514).
- [Input preparation and reconstruction adapter](../tools/build_pulsemech_compute_bounded_execution_inputs_v0.py).
- [Bounded execution capture](../tools/capture_pulsemech_compute_bounded_execution_v0.py).
- [Distinct downstream result consumer](../tools/consume_pulsemech_compute_bounded_result_v0.py).
- [Independent checker and reconstruction entrypoint](../tools/check_pulsemech_compute_bounded_execution_v0.py).
- [Manual non-active reference workflow](../.github/workflows/pulsemech_compute_bounded_execution_reference.yml).
- [Permanent connected regression](../tests/test_pulsemech_compute_bounded_execution_v0.py).

The actual checker/consumer outcomes remain:
...
Exactly six checker/consumer occurrences remain distinct. A successful consumer
does not convert checker exit `1` or `2` into ALLOW.

Both completed reconstruction processes reproduced the acquired
322,567-byte, 12-member output byte-for-byte. All ten preserved negative or
preservation scenarios reproduced their intended rejection or output-preservation
boundary. The review found no actionable preservation, replay or bounded-claim
defect.
...
The three non-active candidate values are true only for this bounded reference.
The generic `authority_binding_complete` and `decision_closure_complete` fields
remain false. The whole runtime packet remains partial, and the projection
collector is not a seventh fully observed subject process.

The current tools-test manifest contains 154 unique program paths. The
153-program count in the Step 5A section above remains the historical count of
that earlier execution.
...
The acceptance record authorizes this canonical documentation synchronization.
The final #2875 work-order completion record remains a separate issue event after
the canonical surfaces and their own checks are complete.

### Remaining implementation and operational boundary

The runtime-observation contract, matching validator, bounded historical
producer and Step 5A source-aware report/relation path are implemented. Step 5B
now supplies one actual owner-dispatched observation with verified source,
finite extent, downstream consumption, exact preservation and independent
replay.

That result closes only the declared six-process boundary. It does not make the
partial historical Step 4B packet or the whole runtime surface complete.
...
+ completed Step 5B six-process observation, preservation and replay
→ wider whole-runtime live/current-run observation
→ remaining fixed-source/runtime comparison obligations
→ complete Step 5 observed proof
→ Step 6 per-axis resource measurement
→ Step 7 separate policy promotion decision
```

The future operational lane requires exact current-run source capture before
its authority decision, followed by runtime-packet production, planned-observed
transition relation and transition-path verification under declared policy.
The historical post-run snapshot does not satisfy that pre-decision requirement
by reclassification. Unavailable information remains unavailable, not zero or
inferred compute-resource use.
...
existing roles. Documentation synchronization records completed evidence; it
does not create another validator, new authority input or additional
implementation condition.

Step 4A documentation closure was accepted through PR #2862 and work order
#2856 remains complete. Step 4B documentation completed through #2869 and
#2864 is closed. Work order #2870 is closed for bounded Step 5A. Under #2875,
the Step 5B implementation, acquisition, preservation and independent replay
evidence is accepted and canonical synchronization is authorized.

The final bounded #2875 completion record remains separate until all canonical
surfaces and this documentation PR's own checks and review are complete. This
index update neither predeclares that closure nor performs a new proof execution.
