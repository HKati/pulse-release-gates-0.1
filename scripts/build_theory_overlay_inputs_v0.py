...
def extract_inputs_and_params(raw: Any) -> Tuple[Dict[str, Any], Optional[Dict[str, Any]], List[str]]:
    ...
    # Default contract-shaped inputs
    ...
    # Extract numeric inputs (never include params under inputs in the output).
    ...
    # Note: schema/contract requires T OR lnT numeric; we do not fabricate values here.
...
def main() -> int:
    ...
    # source_kind: explicit flag > validated raw metadata > sensible fallback
    ...
    # Compute digest over inputs only (provenance/timestamps must not affect it).
    try:
        ...
        # If canonicalization fails (e.g., NaN/Inf sneaks in), fail-closed by zeroing inputs.
    ...
    # Emit top-level params only (never under inputs)
