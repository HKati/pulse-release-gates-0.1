...
validator's non-zero exit code. Do not treat direct gate-evaluator
output as canonical for that case.
...
When this study change is the only scoped update, use text equivalent to
the following in the repo workflow:

### Commit message

docs(studies): clarify schema-invalid repro workflow

### PR title

Clarify schema-invalid authority-boundary repro workflow

### PR / merge summary

```markdown
## Summary
- document that `schema_invalid_non_boolean_gate.status.json` must rebind
  `STATUS` and run only `tools/validate_status_schema.py`
- state that the reproduction must stop on the validator's non-zero exit
  code and must not continue into policy materialization or
  `check_gates.py`
